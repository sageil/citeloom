...
function readBasename(sourceFile) {
...
function readContentCountLabel(value, available) {
  return available ? formatCount(value) : "Waiting";
}

function formatDocumentLocationCount(filename, count) {
  if (count === null) {
    return "N/A";
  }
...
function formatElapsedDuration(startedAt, currentTimeMs) {
...
function buildCollections(facets) {
  const collections = [{
    count: facets.total,
    key: "all",
    label: "All documents",
...
function readCollectionLabel(collection) {
  if (collection === "all") {
...
function readDocumentStatusCopy(document) {
...
function combineStatusDetails(...values) {
  const details = [];
  for (const value of values) {
    if (value !== null && value !== "") {
      details.push(value);
...
function readMediaProgressDetail(document, kind) {
...
function buildPhaseStages(document) {
...
function readNextSelectedDocument(current, catalog) {
  if (current !== null) {
...
function readRetryPhase(document) {
  if (document.phase === "discovered") {
