...
Back up PostgreSQL and source content before a procedure that can change persistent data.
Read the complete procedure, including its recovery steps, before you start.
...
The schema source is `src/database/schema.ts`.
The greenfield migration is `drizzle/0000_citeloom_schema.sql`, and `drizzle/bootstrap.sql` owns initial reference data and settings.
The database workflow in [Contributing](../CONTRIBUTING.md) applies to changes in these files.
...
As an administrator, open **Settings > Object storage** to change between a mounted filesystem and S3-compatible storage such as SeaweedFS.
...
Reverse migration is the rollback path.
Cleanup of the old backend is a separate explicit operation.
...
The apply operation contacts only PostgreSQL.
It changes the mode, stages the OAuth configuration, increments the settings version, deletes local sessions, and writes the recovery audit event in one transaction.
...
If the apply command fails, use its error ID to inspect the application logs.
Resolve the recorded cause, and then rerun the report.
The transaction does not leave a partially committed mode change.
...
Use the document controls in the web application to reindex after a setting changes searchable content.
These settings include the embedding model, dimensions, search text format, document section method, and search-index name.
...
Open **System health** to inspect the current application state, configured models, provider capacity, worker capacity, and scheduling telemetry.
The page also shows recent model-request metrics when those metrics are enabled.
The page reads the saved runtime snapshot without contacting every provider.
As an administrator, select **Run diagnostics** to check the database, document service, claim verifier, and provider model availability without model inference.
...
As an administrator, open **Error reports** to review sanitized failures recorded in PostgreSQL.
...
Select **Purge logs** to remove the operational error rows visible to the current workspace.
